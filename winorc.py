...
if __name__ == '__main__':
    if 2 <= len(sys.argv) <= 3:
        lang = 'en-US' if len(sys.argv) == 2 else sys.argv[1] #lang = 'zh-hans-cn' if len(sys.argv) == 2 else sys.argv[1]
        result = recognize_file(sys.argv[-1], lang)
        #pprint.pprint(result, width=128)
        print('*'*20)
        print(result.keys())
        text_file = open("C:/python_programs/Practice/test.txt","w")
        text_file.write(str(result.get('text')))
        text_file.close()
        print('text out')

    else:
        print('usage: %s [language=en-US] filename' % sys.argv[0]) #print('usage: %s [language=zh-hans-cn] filename' % sys.argv[0])
